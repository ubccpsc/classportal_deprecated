...
export default class LoginController {
  ...
  public static login(csid: string, sid: string, authcode: string, parentCallback: any) {
    // save these variables in the outer function, for easy access from any inner function in the waterfall.
    let persistUsername: string;
    let persistAdmin: boolean;
    let persistGithubToken: string;
      ...
      function check_user_exists_or_create_user(callback: any) {
        ...
        Helper.readJSON(persistAdmin ? 'admins.json' : 'students.json', function (error: any, data: any) {
          if (!error) {
            ...
            let file = data;
            let userIndex: number = _.findIndex(file, { 'username': persistUsername });
            ...
            if (userIndex >= 0) {
              ...
              if (persistAdmin) {
                ...
                let newUserIndex = _.findIndex(file, { 'csid': csid, 'sid': sid });
                if (newUserIndex >= 0) {
  ...
  public static logout(username: string, callback: any) {
  ...
  public static checkRegistration(csid: string, sid: string, parentCallback: any) {
   ...
   */
  public static loadStudentPortal(username: string, parentCallback: any) {
    // for efficiency, we load and save each file in its entirety just once, here.
    let studentsFile: any;
    let teamsFile: any;
    let gradesFile: any;
    let deliverablesFile: any;

    let myStudentIndex: number;
    let myTeamIndex: number;
    let myGradesIndex: number;
    let namesArray: any[] = [];
    let appsArray: any[] = [];
    ...
    async.waterfall([
      ...
      function get_names_array(callback: any) {
        ...
        if (studentsFile[myStudentIndex].hasTeam === true) {
          for (let index = 0; index < teamsFile[myTeamIndex].members.length; index++) {
            let sid = teamsFile[myTeamIndex].members[index];
            let teammateStudentIndex: number = _.findIndex(studentsFile, { 'sid': sid });
            let teammateStudentFile: any;

            if (teammateStudentIndex < 0) {
              teammateStudentFile[index] = 'null';
            } else {
              teammateStudentFile = studentsFile[teammateStudentIndex];
              let name: string = teammateStudentFile.firstname + ' ' + teammateStudentFile.lastname;
          ...
          for (let index = 0; index < studentsFile.length; index++) {
            if (studentsFile[index].hasTeam === false) {
              let studentName: string = studentsFile[index].firstname + ' ' + studentsFile[index].lastname;
   ...
   * @returns object containing files
   */
  public static loadAdminPortal(username: string, parentCallback: any) {
    let adminsFile: any;
    let myAdminIndex: number;
    let studentsFile: any;
    let teamsFile: any;
    let gradesFile: any;
    let deliverablesFile: any;
    let namesArray: any[] = [];
    let appsArray: any[] = [];
    let autodash = config.autotest_auth;
